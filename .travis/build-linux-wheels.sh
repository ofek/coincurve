...
if [[ "$PLAT" == "manylinux2010_x86_64" ]]; then
    curl -O https://nixos.org/releases/patchelf/patchelf-0.10/patchelf-0.10.tar.bz2 && tar -xjpf patchelf-*.tar.bz2 && cd patchelf* && ./configure > /dev/null && sudo make install > /dev/null && cd ..
