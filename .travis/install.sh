...
if [[ $TRAVIS_OS_NAME == "osx" ]]; then
	...
	if [[ "${TRAVIS_PYTHON_VERSION}" == "3.5" ]]; then
		...
		sed -i "" 's#print(" -- creating symlink to certifi certificate bundle")#print(" -- creating symlink to certifi certificate bundle", relpath_to_certifi_cafile, openssl_cafile, certifi.where())#' install_certificates.command
